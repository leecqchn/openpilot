...
void NvgWindow::drawLaneLines(QPainter &painter, const UIScene &scene) {
  if (!scene.end_to_end) {
    ...
    for (int i = 0; i < std::size(scene.lane_line_vertices); ++i) {
      painter.setBrush(QColor::fromRgbF(0.0, 1.0, 0.0, scene.lane_line_probs[i]));
  ...
  bg.setColorAt(0, scene.end_to_end ? redColor() : QColor(255, 215, 0));
  bg.setColorAt(1, scene.end_to_end ? redColor(0) : QColor(255, 215, 0, 0));
  painter.setBrush(bg);
